...
const FIELDS = {
  ...
  COLOR: { validate: (val) => val, options: _values__WEBPACK_IMPORTED_MODULE_0__["COLOR_SCHEMES"] }
};
  ...
  browser.storage.local.get(settingsKeys)
    .then((values) => {
      for (let key in values) {
        ...
        field.default = _values__WEBPACK_IMPORTED_MODULE_0__["DEFAULT_SETTINGS"][key].default;
        el.value = values[key];
      }
    });
}
...
function resetToDefaults () {
  browser.storage.local.set(_values__WEBPACK_IMPORTED_MODULE_0__["DEFAULT_SETTINGS"]);
}
